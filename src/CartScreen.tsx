...
export default function CartScreen({ items, walletBalance, onBack, onUpdateQty, onRemove, onPay, onPaidReturn, onOpenWallet }: Props) {
  const total = items.reduce((s, i) => s + i.price * i.qty, 0)
  const [pay, setPay] = useState<PayState>({ kind: 'idle' })
  const paidTotalRef = useRef(0)
      ...
      <div className="screen" style={{ background: 'linear-gradient(180deg,#1a0306 0%, #0a0a0a 60%)' }}>
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 24, textAlign: 'center' }}>
          ...
          <div style={{
            ...
            <span style={{ color: '#fff', fontWeight: 800 }}>{formatINR(paidTotalRef.current)}</span>
          </div>
        </div>
        <div style={{ padding: '0 18px 36px' }}>
          <button className="btn btn-red" onClick={done}>Back to feed</button>
        </div>
        <style>{`
  ...
  return (
    <div className="screen slide-in" style={{ background: '#0a0a0a', paddingBottom: 96 }}>
      ...
      {items.length === 0 ? (
        ...
        <div className="no-scrollbar" style={{ flex: 1, overflowY: 'auto', padding: '4px 16px 170px' }}>
          {items.map(item => (
            <div key={item.id} style={{
              display: 'flex', gap: 12, padding: '12px 0',
              borderBottom: '1px solid rgba(255,255,255,0.08)',
            }}>
              <ProductImage src={item.image} alt={item.name} style={{ width: 64, height: 64, borderRadius: 12, objectFit: 'cover', flexShrink: 0 }} />
